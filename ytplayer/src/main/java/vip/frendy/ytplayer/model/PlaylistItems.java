...
public class PlaylistItems implements Serializable {
    ...
    public static void save(Context context, PlaylistItems data) {
        if(context != null) {
            if(data != null) data.setTimestamp(System.currentTimeMillis());
    ...
    public static ArrayList<PlaylistItems> queryAllDesc(Context context) {
        if(context != null) {
            return DBManager.getInstance(context).query(
                    new QueryBuilder<>(PlaylistItems.class).appendOrderDescBy(COL_TIME));
        }
        return new ArrayList<>();
    }
